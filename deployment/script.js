...
class WeatherAlertMonitor {
    constructor() {
        ...
        // Use the current protocol and hostname for the server API to work from remote computers
        // Add port 8000 for local development, remove for production
        const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
        const port = isLocalhost ? ':8000' : '';
        this.SERVER_API = `${window.location.protocol}//${window.location.hostname}${port}/api`;
        ...
        this.WEATHER_APIS = {
            ...
            'ca': {
                ...
                baseUrl: 'https://dd.weather.gc.ca/alerts/cap/',
                alertsEndpoint: '', // We'll build this dynamically
                supported: true
            },
            'mx': {
                name: 'CONAGUA (Mexico)',
                baseUrl: 'https://smn.conagua.gob.mx/feeds/',
                alertsEndpoint: 'warningMX.xml',
                supported: true
            }
        };
...
if (alertsData && alertsData.features && alertsData.features.length > 0) {
    ...
    updateAlertCounts() {
        ...
        if (document.getElementById('severeCount')) {
            document.getElementById('severeCount').textContent = counts.warning;
        }
        if (document.getElementById('moderateCount')) {
            document.getElementById('moderateCount').textContent = counts.watch;
        }
        if (document.getElementById('minorCount')) {
            document.getElementById('minorCount').textContent = counts.advisory;
        }
        // No element for 'none' in the summary circles
    }
    ...
    updateLocationsList() {
        ...
        // Add click listeners to alert-card-location-name to zoom map to location
        document.querySelectorAll('.alert-card-location-name').forEach(nameEl => {
            nameEl.addEventListener('click', (e) => {
                e.stopPropagation();
                const card = nameEl.closest('.alert-card');
                if (!card) return;
                const locationId = card.dataset.locationId;
                const location = this.locations.find(loc => loc.id === locationId);
                if (location && this.map && location.coordinates && location.coordinates.length === 2) {
                    // Smooth zoom animation with easing
                    this.map.flyTo(location.coordinates, 14, {
                        duration: 1.5,
                        easeLinearity: 0.25
                    });
                }
    ...
    updateMarkers() {
        ...
        if (this.locations.length === 0) {
            console.log('No locations to create markers for');
            return;
        }
        ...
        this.locations.forEach((location, index) => {
            console.log(`Processing location ${index + 1}:`, location.nickname, location.coordinates);
            
            if (!location.coordinates || !Array.isArray(location.coordinates) || location.coordinates.length !== 2) {
                console.error(`Invalid coordinates for ${location.nickname}:`, location.coordinates);
                return;
            }
            
            const [lat, lng] = location.coordinates;
            if (isNaN(lat) || isNaN(lng)) {
                console.error(`Invalid lat/lng for ${location.nickname}:`, lat, lng);
                return;
            }
            
            const alertInfo = this.alertSeverity[location.currentAlert];
            if (!alertInfo) {
                console.error(`No alert info for ${location.nickname} with alert type:`, location.currentAlert);
                return;
            }
            ...
            try {
                ...
                console.error(`Error creating marker for ${location.nickname}:`, error);
            }
        });
    ...
    async fetchCanadianWeatherData(location) {
        ...
        // Try to extract province code from address or state field
        let province = '';
        if (location.state) {
            province = location.state.toUpperCase();
        } else if (location.address) {
            // Try to match province code in address
            const match = location.address.match(/,\s*([A-Za-z]{2})\s*[A-Za-z0-9 ]*$/);
            if (match) province = match[1].toUpperCase();
        }
        // List of valid province codes for Environment Canada
        const validProvinces = [
            'AB','BC','MB','NB','NL','NS','NT','NU','ON','PE','QC','SK','YT'
        ];
        if (!validProvinces.includes(province)) {
            console.warn(`Could not determine valid province for Canadian location: ${location.nickname}`);
            location.currentAlert = 'none';
            location.currentAlertDescription = 'No valid province found for this location.';
            return location;
        }
        const feedUrl = `https://dd.weather.gc.ca/alerts/cap/${province}.xml`;
        try {
            const response = await fetch(feedUrl);
            if (!response.ok) throw new Error(`Failed to fetch Environment Canada feed: ${response.status}`);
            const xmlText = await response.text();
            // Parse XML
            const parser = new DOMParser();
            const xmlDoc = parser.parseFromString(xmlText, 'application/xml');
            const entries = Array.from(xmlDoc.getElementsByTagName('entry'));
            // Find alerts relevant to this location (by city or region in summary/title)
            const city = location.city ? location.city.toLowerCase() : '';
            const nickname = location.nickname ? location.nickname.toLowerCase() : '';
            let relevantEntry = null;
            for (const entry of entries) {
                const title = entry.getElementsByTagName('title')[0]?.textContent?.toLowerCase() || '';
                const summary = entry.getElementsByTagName('summary')[0]?.textContent?.toLowerCase() || '';
                if ((city && (title.includes(city) || summary.includes(city))) ||
                    (nickname && (title.includes(nickname) || summary.includes(nickname)))) {
                    relevantEntry = entry;
                    break;
                }
            }
            if (relevantEntry) {
                const alertTitle = relevantEntry.getElementsByTagName('title')[0]?.textContent || 'Weather Alert';
                const alertSummary = relevantEntry.getElementsByTagName('summary')[0]?.textContent || '';
                location.currentAlert = 'alert';
                location.currentAlertDescription = alertTitle + ': ' + alertSummary;
                location.shortForecast = alertTitle;
                location.detailedForecast = alertSummary;
            } else {
                location.currentAlert = 'none';
                location.currentAlertDescription = 'No active alerts for this location.';
                location.shortForecast = 'No alerts';
                location.detailedForecast = 'No active weather alerts.';
            }
            location.lastWeatherUpdate = new Date().toISOString();
            return location;
        } catch (error) {
            console.error('Error fetching or parsing Environment Canada alerts:', error);
            location.currentAlert = 'none';
            location.currentAlertDescription = 'Error fetching Canadian weather data.';
            location.shortForecast = 'Weather data error';
            location.detailedForecast = 'Could not fetch Environment Canada alerts.';
            location.lastWeatherUpdate = new Date().toISOString();
            return location;
        }
    }
    ...
    async fetchMexicanWeatherData(location) {
        ...
        const feedUrl = 'https://smn.conagua.gob.mx/feeds/warningMX.xml';
        try {
            const response = await fetch(feedUrl);
            if (!response.ok) throw new Error(`Failed to fetch CONAGUA feed: ${response.status}`);
            const xmlText = await response.text();
            // Parse XML
            const parser = new DOMParser();
            const xmlDoc = parser.parseFromString(xmlText, 'application/xml');
            const items = Array.from(xmlDoc.getElementsByTagName('item'));
            // Try to match by state or city in title/description
            ...
            for (const item of items) {
                ...
                if ((state && (title.includes(state) || description.includes(state))) ||
                    (city && (title.includes(city) || description.includes(city))) ||
                    (nickname && (title.includes(nickname) || description.includes(nickname)))) {
                    relevantItem = item;
                    break;
                }
            }
            if (relevantItem) {
                const alertTitle = relevantItem.getElementsByTagName('title')[0]?.textContent || 'Weather Alert';
                const alertDescription = relevantItem.getElementsByTagName('description')[0]?.textContent || '';
                location.currentAlert = 'alert';
                location.currentAlertDescription = alertTitle + ': ' + alertDescription;
                location.shortForecast = alertTitle;
                location.detailedForecast = alertDescription;
            } else {
                location.currentAlert = 'none';
                location.currentAlertDescription = 'No active alerts for this location.';
                location.shortForecast = 'No alerts';
            ...
            console.error('Error fetching or parsing CONAGUA alerts:', error);
            location.currentAlert = 'none';
            location.currentAlertDescription = 'Error fetching Mexican weather data.';
            location.shortForecast = 'Weather data error';
            location.detailedForecast = 'Could not fetch CONAGUA alerts.';
            location.lastWeatherUpdate = new Date().toISOString();
            return location;
        }
    }
...
function setAddLocationVisibility(role) {
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (!session.token) {
        showLoginModal();
        setAddLocationVisibility('none');
    } else {
        hideLoginModal();
        setAddLocationVisibility(session.role);
    }
});
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (resp.ok && data.token) {
                ...
                hideLoginModal();
                setAddLocationVisibility(data.role);
                window.location.reload(); // reload to ensure app picks up session
            } else {
